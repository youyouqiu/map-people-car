...
const initialValues = {
  ...
  addOilAmountThreshold: '8',// 加水量阈值
  addOilTimeThreshold: '40',// 加水时间阈值
  seepOilTimeThreshold: '35',// 漏水时间阈值
  seepOilAmountThreshold: '6',// 漏水量阈值
};

const WaterParam = (props: IProps) => {
  ...
  const [currentShape, setCurrentShape] = useState(1);// 当前水箱形状
  ...
  const getParamStatusFun = async () => {
    const param = {
      functionPage: 'water_sensor',// 功能页面
      monitorId: props.monitorId
    }
  ...
  const calculateCalibration = async () => {
    ...
    formRef.current?.validateFields(paramArr).then(async (values: any) => {
      ...
      const defaultParam: any = {
        ...
        sensorOutId: props.sensorOutId,// 传感器外设id：65(0x41水量主)、66(0x42水量副)、67(0x43水量主)、68(0x44水量副)、112(0x70载重1#)、113(0x71载重2#)
        theoryVolume: '',// 理论容积：L
      }
      await getTheoryVolume(values);
  ...
  const waterBasicParam = [{
    ...
    component: <Select onChange={(value) => { setCurrentShape(Number(value)) }} bordered={false} getPopupContainer={() => getSelectContainer(`waterParam_${props.sensorOutId}`)}>
  ...
  if (currentShape === 1) {
    ...
    waterBasicParam.splice(5, 0, newParam[0], newParam[1]);
  }
  ...
  return (
    <>
      <Form
        ...
        onFinish={formSubmit}
        id={`waterParam_${props.sensorOutId}`}
        style={{ paddingTop: 5 }}
        className={styles.formWrapper}
      >
        <TableForm dataSource={waterBasicParam} column={4} header='水箱基本参数' />
