...
const ReadCalibrationData = (props: IProps) => {
  ...
  const getParamStatusFun = async () => {
    const param = {
      functionPage: 'oil_calibration',// 功能页面
  ...
  const sendSocketFun = (type: number) => {
    ...
    globalSocket.subscribeAndSend('/user/queue/oil_calibration', (res: any) => { socketCallBack(res, type) }, '/app/monitor/calibration', header, requestStr, true);
  }
  ...
  return (
    <Modal
      ...
      footer={[<Button key='readCalibrationData' onClick={() => props.changeVisible('')}>取消</Button>]}
    >
