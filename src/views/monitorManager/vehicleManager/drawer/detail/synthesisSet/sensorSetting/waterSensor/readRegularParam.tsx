...
interface IProps {
  ...
  updateFun: Function,
  sensorOutId: number,// 传感器外设id：65(0x41水量主)、66(0x42水量副)
}
...
const ReadRegularParam = (props: IProps) => {
  ...
  const getSensorBindInfoFun = async () => {
    ...
    if (result && result.length > 0) {
      ...
      } else if (props.sensorOutId === 68 && result[1]) {// 副水箱
  ...
  const sendSocketFun = (type: number) => {
    ...
    globalSocket.subscribeAndSend('/user/queue/water_sensor', (res: any) => { socketCallBack(res, type) }, '/app/monitor/sensor', header, requestStr, true);
  }
  ...
  return (
    <Modal
      ...
      footer={[<Button key="readLoadRegularParam" onClick={() => props.changeVisible('')}>取消</Button>]}
    >
      <Form
        ref={formRef}
