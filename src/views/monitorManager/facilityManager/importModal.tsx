...
function ImportModal(props: IProps) {
  ...
  function downloadAddModelFun() {
    setDownLoading('add');
    downloadFile('/api/sa/facility/downLoad/template.xls?type=0', 'get', '设施新增模板.xls', undefined, () => { setDownLoading('') });
  }

  // 下载绑定模板文件
  function downloadBindModelFun() {
    setDownLoading('bind');
    downloadFile('/api/sa/facility/downLoad/template.xls?type=1', 'get', '设施绑定模板.xls', undefined, () => { setDownLoading('') });
  }

  // 下载失败列表
  function downloadErrorList(type: string) {
    setErrorLoading(true);
    downloadFile(`/api/mb/facility/exportInfoFacility/${type}`, 'get', '失败列表.xls', undefined, () => { setErrorLoading(false) });
  }
  ...
  return (
    ...
    >
      <Row className={styles.importRow} gutter={12}>
        ...
        <Col span={20}>
          <Upload
            ...
            action='/api/saoverload/facility/import/facility'
            fileList={fileList}
      ...
      <Row className={styles.importRow} gutter={12}>
        ...
        <Col span={20}>
          <Button type='link' className={styles.downLink} loading={downLoading === 'add'} onClick={downloadAddModelFun}>设施新增模板</Button>
          <Button type='link' className={styles.downLink} loading={downLoading === 'bind'} onClick={downloadBindModelFun}>设施绑定模板</Button>
        </Col>
      </Row>
      <Row className={styles.importRow} gutter={12}>
        <Col span={20} push={3}>
