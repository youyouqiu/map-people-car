...
function ImportModal(props: IProps) {
  ...
  function downloadAddModelFun() {
    setDownLoading('add');
    downloadFile('/api/mb/professional/template.xls?type=0', 'get', '人员新增模板.xls', undefined, () => { setDownLoading('') });
  }

  // 下载绑定模板文件
  function downloadBindModelFun() {
    setDownLoading('bind');
    downloadFile('/api/mb/professional/template.xls?type=1', 'get', '人员绑定模板.xls', undefined, () => { setDownLoading('') });
  }

  // 下载失败列表
  function downloadErrorList(importType: string) {
    setErrorLoading(true);
    downloadFile(`/api/mb/professional/exportInfoProfessionals/${importType}`, 'get', '失败列表.xls', undefined, () => { setErrorLoading(false) });
  }
  ...
  return (
    ...
    >
      <Row className={styles.importRow} gutter={12}>
        ...
        <Col span={20}>
          <Upload
            ...
            action='/api/mooverload/professional/import'
            fileList={fileList}
      ...
      <Row className={styles.importRow} gutter={12}>
        ...
        <Col span={20}>
          <Button type='link' className={styles.downLink} loading={downLoading === 'add'} onClick={downloadAddModelFun}>人员新增模板</Button>
          <Button type='link' className={styles.downLink} loading={downLoading === 'bind'} onClick={downloadBindModelFun}>人员绑定模板</Button>
        </Col>
      </Row>
      <Row className={styles.importRow} gutter={12}>
        <Col span={20} push={3}>
