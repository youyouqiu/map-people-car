...
import { checkUserName } from '@/server/orgMgm';
import { enterpriseAdd, enterpriseUpdate } from '@/server/enterpriseUser';
...
interface IProps {
    ...
    enterpriseId?: string | undefined //企业id
}
...
class UserAddDrawer extends Component<IProps, IState, any> {
    ...
    UNSAFE_componentWillReceiveProps(nextPros: IProps) {
        ...
        if (type == 0 && loginUser) {//新增时候设置授权截止日期,所属企业
            this.setState({
                loading: true
            }, () => {
                this.authorizationDate(loginUser, enterpriseId);
            });
        }
     ...
     * 初始化授权日期
     */
    authorizationDate = (loginUser: IUserDetails, organizationId: string | undefined) => {
        if (!loginUser) {
            return;
        }
    ...
    async addRequest(values: any) {
        const datas = await enterpriseAdd(values);
        if (datas) {
    ...
    async upDateRequest(params: any) {
        const datas = await enterpriseUpdate(params);
        if (datas) {
