...
class PathAddDrawer extends Component<IProps, IState> {
  static defaultProps = {
    type: 0,
  };
  ...
  UNSAFE_componentWillReceiveProps(nextPros: IProps) {
    ...
    if (type == 0 && visible) {
      ...
      this.formRef.current?.setFieldsValue({ roadWidth: 20 });
    }
  ...
  formatFormAlarmData = (values: any) => {
    ...
    this.state.detailModeList.forEach((item: any) => {
      const stayOvertime = {}, leaveTime = {}, gatherTime = {}, gatherScope = {}, workSpeed = {};
              ...
              <div style={{ display: 'flex' }}>
                ...
                <div className="form">
                  <Form ref={this.formRef} style={{ width: '450px' }} initialValues={this.state.initValue}>
                    ...
                    {/* 这个组件每次调用后需要销毁 */}
                    {/* {visible ? (
                              <DynamicForm modeList={this.state.modeList} modeDetail={this.state.detailModeList} setModeList={this.setModeList} />
                          ) : null} */}
                  </Form>
                </div>
                {/* 地图 */}
                <div className="map" style={{ width: '700px', marginLeft: '14px', height: '410px' }}>
                  ...
                  {this.props.type == 0 ? (
                    visible ? (
                      <AddMap
                        type='polyline'
                        toolOptions={['polyline', 'GPS']}
                        fences={this.state.sectionLonLat}
                        calcType={1} // 1计算长度
                        setArea={this.setDistance}
