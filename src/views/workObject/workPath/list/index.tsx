...
// import LeftTree, { SelectParams } from '../tree';
...
class WorkPath extends Component<Iprops, IState> {
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      rootId: '',
      queryArgs: {
        workType: 1, //1 作业线路 2 作业区域  3清运区域
      },
      rowId: '',
      curRow: '',
      refreshCount: 0,
  ...
  async delMore() {
    ...
    const datas = await deleteWorkObjectBatch<any>(params);
    if (datas) {
   ...
   * 节点选中回调
   */
  onNodeSelect = (value: any) => {
    console.log(value);
    this.getAllEnterpriseId(value);
    console.log('allIds', this.allIds);
    this.setState({
      enterpriseIds: this.allIds,
    });
    this.allIds = [];
  };
  /**
   * 获取点击节点下所有的enterpriseId
   */
  allIds: any[] = [];
  getAllEnterpriseId = (value: any) => {
    if (value.enterpriseId) {
      if (!this.allIds.includes(value.enterpriseId)) {
        this.allIds.push(value.enterpriseId);
      }
    }
    if (value.children && value.children.length > 0) {
      value.children.forEach((item: any) => {
        this.getAllEnterpriseId(item);
      });
    }
  };
  /**
  ...
  render() {
    const { addVisible, type, selectedRowKeys, queryArgs, detailVisible, initOrgId, rootId, rowId, orgName } = this.state;
...
export default connect((state: AllState) => ({
  rootOrgId: state.root.userMessage.organizationId,
