...
import { fuzzySearchTree } from '@/server/workManagement';
...
class Index extends Component<IProps, IState, any> {
  ...
  getTreeDataFun = async (param?: string) => {
    ...
    if (result) {
      ...
      const handleRes = result.map((item: INodeItem) => {
      ...
      const treeData = getTreeRoot(handleRes);
      this.setState({
        treeData: treeData,
        defaultExpandedKeys,
        loading: false,
