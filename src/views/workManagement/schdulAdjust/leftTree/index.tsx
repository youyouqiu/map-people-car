...
class Index extends Component<IProps, IState, any> {
  ...
  onLoadData = async (treeNode: INodeItem) => {
    ...
    const { inputValue } = this.state
    if (inputValue && this.state.type == 1) return
  ...
  onExpand = (
    ...
    if (!info.expanded) {
      const newValue = this.state.expandedKeys.filter((item) => {
      ...
      const newValue = [...this.state.expandedKeys, info.node.key]
      this.setState({
        expandedKeys: newValue,
      })
    }
      ...
      <div className={styles.treeWrapper}>
        <div
          className={[
            ...
            isShowselect ? styles.showArrow : null,
          ].join(' ')}
        >
          ...
          <Tree
            key={this.state.inputValue + this.state.type}
            ref={this.localRef}
            checkable={false}
