...
import { getWorkTree } from '@/server/auditsManagement';
...
class OrgTree extends Component<IProps, IState, any> {
  ...
  getOrgTree = async (param?: string) => {
    const result = this.props.customizeTree
      ...
      : await getWorkTree<Array<IOrgTree>>({ keyword: param });
    if (result) {
        ...
        <div className={styles.treeContainer}>
          <div className={styles['search-input']}>
            <TrimInput
              ...
              // value={inputValue}
              onChange={(value: any) =>
                this.serarchTree(value)
              }
              maxLength={30}
            />
          </div>
          {treeData.length === 0 && (
            <div className={styles.noTreeData}>未找到标段</div>
          )}
          <Tree
