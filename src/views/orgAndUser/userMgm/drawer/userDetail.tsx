...
import AssignGroupDrawer from './assignGroup';
...
class UserDetailDrawer extends Component<IProps, IState, any> {
  ...
  async getUserMsg(userId: string) {
    ...
    const datas = await getUserDetail<IUserInfo>(params);

    if (datas) {
      datas.gender = datas.gender == '1' ? '男' : '女';
      datas.isActive = datas.isActive == '1' ? '启用' : '停用';
        ...
        {/* 分配分组抽屉 */}
        {
          assignGroupVisible != undefined && (
            <AssignGroupDrawer
              ...
              closeDrawer={this.closeAssignGDrawer}
              userId={userId}
