...
import { getUserList, userRoles, delUserMore, delUser } from '@/server/orgMgm';
 ...
 * 当前页面权限
 */
const permission = getCurrentUserPermission('4_system_user_list');
...
class UserList extends Component<any, IState, any>{
      ...
      render: (value: any, record: IUserInfo) => {
        ...
        return (
          <>
            ...
            >修改</Button>|
            {
              ((record.username === 'admin' || this.userId === record.id) && this.hasPermission('删除')) ?
                (
                  <Button
                    disabled
                    type="link"
...
*/
  ...
  async getLoginUser() {
    ...
    for (let key in datas) {
      if (key == 'organizationId') {
        values[key] = loginUser.organizationId
      } else {
   ...
   * 获取当前选中组织树节点
   */
  getSelectNode = (selectedNode: any) => {
    const { loginUser } = this.state;
    if (!selectedNode) { return; }

    loginUser.organizationId = selectedNode.id;
   ...
   * 新增获取所属组织Id
   */
  queryOrgId = (orgId: any) => {
    const { loginUser } = this.state;
    loginUser.organizationId = orgId;
    this.setState({
      loginUser
    })
  }
  ...
  async delMore() {
    ...
    // }
    const datas = await delUserMore(selectedRowKeys);
    if (datas) {
  ...
  async delOne(id: string) {
    ...
    const datas = await delUser(params);
    if (datas) {
    ...
    return (
      <div style={{ height: '100%' }}>
        <Table
          ...
          tree={<OrgTree treeNodeClick={this.getSelectNode} queryOrgId={this.queryOrgId} />}
          btnGroup={[
            <Button
