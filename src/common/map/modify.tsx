...
const Map: React.FC<IProps> = (props) => {
  const { setArea, setLngLat, setFacility, setAdcode, setFenceType, toolOptions, fences, calcType, type, path, detailFacility, initAdCode } = props
  ...
  useEffect(() => {
    ...
    if (getArrDimension(path) == 3) {
      ...
      path.forEach((item: any) => {
        drawPolygon(cloneDeep(item as Lnglat), true)
   ...
   */
  const drawPolygon = (path?: Lnglat, noEdit?: boolean) => {
    if (path && path.length > 0) { //修改模式
      const delMarkerPosition = cloneDeep(path[path.length - 1])
      const polygon = new AMap.Polygon({
        ...polygonOptions,
        map: map.current,
        path: path,
      });
      layers.current?.polygonLayers.draw.push(polygon)
      if (!noEdit) {
        const delMarker = addDelMarker(delMarkerPosition, polygon)
        polygon.setExtData({
          delMarker
        })
        polyEditor(polygon)
  ...
  // 画路线
  const drawPolyline = (path?: Lnglat, noEdit?: boolean) => {
    if (path && path.length > 0) {
      const delMarkerPosition = cloneDeep(path[path.length - 1])
      ...
      layers.current?.polylineLayers.draw.push(polyline)
      if (!noEdit) {
        const delMarker = addDelMarker(delMarkerPosition, polyline)
        polyline.setExtData({
          delMarker
        })
        polyEditor(polyline)
  ...
  // 多边形绘制回调
  const onPolygonDrawEnd = useCallback((e: any) => {
    const target = e.obj
  ...
  const polyEditor = (poly: AMap.Polygon | AMap.Polyline) => {
    if (!map.current) return
    polygonEditor.current = new AMap.PolyEditor(map.current, poly);
    polygonEditor.current.on('adjust', (e: any) => {
      onEditorAdjust(e, poly.getExtData().delMarker)
    });
    polygonEditor.current.open()
  };
  // 编辑器adjust回调
  ...
  const generateLine = () => {
    if (GPSPoints.current.start.length > 0 && GPSPoints.current.end.length > 0) {
      map.current?.plugin("AMap.DragRoute", () => {
        ...
        GPSline.current.on('complete', (res: any) => { // 绘制后更新GPSpoints点信息
