...
const { Option } = Select;
import { userTree, roleUserTree } from '@/server/systemManagement';
...
interface IProps {
  ...
  treeCheckBan?: boolean | undefined;
  item?: object;
}
...
function UserTree(props: IProps) {
  ...
  async function getRoleUserTree(param?: string) {
    const { roleId, onGetUserCount, treeEditStatus } = props;
    let count = 0;
  ...
  async function getUserTree(param?: string) {
    const { treeEditStatus, item } = props;

    let userData = undefined;
    if (treeEditStatus) {
      userData = await userTree<Array<INodeItem>>({ keyword: param, queryType });
      if (userData !== undefined) {
        const treeData = getTreeRoot(userData, undefined, '', item);
        setTreeData(treeData)
      }
    }
  }
