...
class ConcretTable<T> extends Component<MyTableProps<T>, IState, any> {
  ...
  async queryTable(data: IQueryParams) {
    const { queryAjax, queryCallback, rowSelection } = this.props
    ...
    if (queryAjax) {
      ...
      if (result) {
        ...
        datas.map((item, index) => item.sort = index + 1 + this.tableQueryArgs.size * (this.tableQueryArgs.page - 1))
        this.setState({
          tableData: datas
        })
  ...
  tableChange = async (page: any, filters: any, sorter: any, extra: any) => {
    ...
    const { pagination, filterdInfoData, filterdInfo } = this.state
      ...
      if (filters[a]) {
        if (filterdInfoData && a in filterdInfoData) {
          if (JSON.stringify(filters[a][0]) === JSON.stringify(filterdInfoData[a])) {
            ...
            filterArgs = {}
            filterArgs[a] = filters[a][0] !== undefined ? filters[a][0] : filters[a]
          }
        } else {
          filterArgs = {}
          filterArgs[a] = filters[a][0] !== undefined ? filters[a][0] : filters[a]
    ...
    if (JSON.stringify(page) !== '{}' && (pagination.pageSize !== page.pageSize || pagination.current !== page.current)) {
      ...
      if (JSON.stringify(filterArgs) === '{}') {
        await this.queryTable(this.pageArgs);
        return
      } else {
        if (filterdInfo !== null) {
          let flag = false;
          Object.keys(filterdInfo).forEach((index) => {
            if (filterdInfo[index][0] === filterArgs[index]) {
              flag = true;
            }
          });
          if (flag) {
    ...
    if (JSON.stringify(sorter) !== '{}') {
      ...
      if (sorter.orrder === 'descend' || sorter.order === 'ascend') {
  ...
  trActive = (x: any) => {
    ...
    if (result.length > 0) {
      ...
      result[0].classList.add('ant-tr-active');
    }
  }
